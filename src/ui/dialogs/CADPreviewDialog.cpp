...
void CADPreviewDialog::setupUi()
{
    ...
    // --- Extrusion type ---
    auto* extrudeGroup = new QGroupBox(tr("Extrusion Type"), rightPanel);
    ...
    rotForm->addRow(tr("Start Angle:"), m_rotAngleStartSpin);
    ...
    rotForm->addRow(tr("End Angle:"), m_rotAngleEndSpin);
    ...
    linForm->addRow(tr("Direction:"), m_linearDirCombo);
    ...
    linForm->addRow(tr("Wideness:"), m_linearWidenessSpin);

    extrudeLayout->addLayout(linForm);
    rightLayout->addWidget(extrudeGroup);

    // --- Display options ---
    auto* displayGroup = new QGroupBox(tr("Display"), rightPanel);
...
{
    // Read control values into params
...
{
    // Rebuild and re-display the shape, which triggers FitAll on the widget
    buildAndDisplayShape();
}
